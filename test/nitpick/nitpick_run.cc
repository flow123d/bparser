...
int main() {
	...
	const size_t buffer_size = sizeof(double) * N * 64;
	...
	for (uint vec_size : vec_sizes) {
		...
		//I would expect vec_size instead of max_vec_size but that certainly does not work because the processor decides to go out of program reach and run memory -LV
		ProcessorBase* processor = ProcessorBase::create_processor(se, max_vec_size, simd_size, arena); //get arena from ExprCase

		std::vector<uint> subset(std::ceil(vec_size / (float)simd_size)); //ex: 32/4 = 8 => 
		std::iota(subset.begin(), subset.end(), 1);					// => {1, 2, 3, 4, 5, 6, 7, 8}
